...
var pubsub = {
    ...
    subscribe: function(topic, callback, receiveLastData) {
        logger.v(TAG, "Add " + topic + " subscribers.");
    ...
    unsubscribe: function(topic, callback) {
        if(! this.subscribers[topic]) {
            logger.v(TAG, "No such topic: " + topic);
            return;
        }
    ...
    publish: function(topic, data) {
        ...
        if(! this.subscribers[topic]) {
            logger.v(TAG, "No subscriber exists for topic: " + topic);
            return;
        }
...
function selfTest() {
    ...
    var callback1 = function(data) {
        logger.v(TAG, "callback1: " + data);
    }
    var callback2 = function(data) {
        logger.v(TAG, "callback2: " + data);
    }
    var callback3 = function(data) {
        logger.v(TAG, "callback3: " + data);
    }
